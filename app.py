...
pub_name = os.getenv(
    'HOSTNAME', ('publisher-' + uuid.uuid4().hex.upper()[0:6]))
...
def api_message():
    ...
    if request.headers['Content-Type'] == 'application/json':
        ...
        if "speed" in message.keys():
            ...
            mqtt.publish(id + 'car/speed', int(speed_perc))
        if "steering" in message.keys():
            angle_perc = message["steering"]
            mqtt.publish(id + 'car/steering', int(angle_perc))
...
def steer_car(angle, direction):
    ...
    wide_angle = ["leicht", "weit", "etwas",
                  "seicht", "ein bisschen", "bisschen"]
    ...
    if direction in left_dir:
        ...
        logger.error(
            f"Invalid Values for Intent: angle: {angle}, direction: {direction}")
...
def accelerate_car(accelerationdirection, speed, speedvalue):
    ...
    if accelerationdirection in backward:
        ...
        logger.error(
            f"Invalid Values for Intent: accelerationdirection: {accelerationdirection}, speed: {speed}, speedvalue: {speedvalue}")
